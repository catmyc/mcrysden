...
Last updated: **2026-08-13 (v1.2.12: band-surface correctness round 2 — full-vector TR unfolding, plot-local triangle rasterization, premultiplied alpha, explicit reciprocal-basis requirement, last-block reciprocal selection, state-persisted orientation; 90 focused tests)**.

Status legend: `[x]` done · `[~]` partly done · `[ ]` todo. "file" = an example input is on hand for immediate test.

Current v1.2.12 verification: `--bands`/`--dos`/`--band-surf` CLI selection, mesh→path interpolation (shifted grids, TR-unfolded half meshes with full-vector negation, Cartesian-conversion rejection), band-surface construction (mesh-derived regions, 2D-plane validation, decode validation), z-buffered rendering (depth occlusion, Fermi domain, plot-local fills, premultiplied alpha, WYSIWYG export orientation), state-file orientation persistence, and the existing band/DOS/bond regressions are covered by a passing 90-test suite.
