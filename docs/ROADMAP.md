...
- [x] Electronic-structure plot selection (v1.2.10): `--bands` shows a band path directly and interpolates a uniform k-mesh along the default canonical route (or the `--kpath` route) via periodic trilinear interpolation of the axis-aligned MP mesh; `--dos` reconstructs the mesh-derived total DOS on request; `--band-surf` builds a true 3D surface plot in rotated k_x/k_y/E axes over the first non-collinear route plane (physical Å⁻¹ k-axis units when the cell is known), with a base plane, Fermi plane, drag-to-rotate, print and PNG/vector export. Band surfaces are limited to 2D systems: a mesh that is not a detected 2D k-grid (bulk 3D meshes) is rejected. By default the 2 bands closest to E_f are plotted; the sidebar band picker selects specific bands or the N closest. Derived data survives GUI re-parses (frame changes, revert).
