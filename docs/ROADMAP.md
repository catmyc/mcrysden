...
Last updated: **2026-08-13 (v1.2.11: band-surface correctness fixes — TR-reduced mesh unfolding, shifted-grid interpolation, mesh-derived surface domains, z-buffered rendering; 84 focused tests)**.

Status legend: `[x]` done · `[~]` partly done · `[ ]` todo. "file" = an example input is on hand for immediate test.

Current v1.2.11 verification: `--bands`/`--dos`/`--band-surf` CLI selection, mesh→path interpolation (shifted grids, TR-unfolded half meshes), band-surface construction (mesh-derived regions, 2D-plane validation, decode validation) and z-buffered rendering (depth occlusion, Fermi domain, WYSIWYG export orientation), opt-in mesh DOS gating, and the existing band/DOS/bond regressions are covered by a passing 84-test suite.
