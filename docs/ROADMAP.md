...
Last updated: **2026-08-13 (v1.2.13: band-surface correctness round 3 — axis-generic TR unfolding with degenerate-axis invariance, parser TR-safety gate for magnetic/SOC runs, non-occluding base plane, single-quad translucent planes; 97 focused tests)**.

Status legend: `[x]` done · `[~]` partly done · `[ ]` todo. "file" = an example input is on hand for immediate test.

Current v1.2.13 verification: `--bands`/`--dos`/`--band-surf` CLI selection, mesh→path interpolation (shifted grids, TR-unfolded half meshes on x/y/z with full-vector negation, TR-breaking magnetic/SOC rejection, Cartesian-conversion rejection), band-surface construction (mesh-derived regions, 2D-plane validation, decode validation), z-buffered rendering (depth occlusion, Fermi domain, plot-local fills, premultiplied alpha, coplanar-band visibility, quad-rasterized translucent planes, WYSIWYG export orientation), state-file orientation persistence, and the existing band/DOS/bond regressions are covered by a passing 97-test suite.
