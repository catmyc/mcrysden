...
- [x] Electronic-structure plot selection (v1.2.10): `--bands` shows a band path directly and interpolates a uniform k-mesh along the default canonical route (or the `--kpath` route) via periodic trilinear interpolation of the axis-aligned MP mesh; `--dos` reconstructs the mesh-derived total DOS on request; `--band-surf` builds a true 3D surface plot in rotated k₁/k₂/E axes (physical Å⁻¹ k-axis units when the cell is known), with a base plane, Fermi plane, drag-to-rotate, print and PNG/vector export. Band surfaces require a 2D k-mesh — exactly two non-degenerate sampling axes, the typical slab calculation; the gate is on the k-mesh sampling, not the physical dimensionality (QE slab outputs usually report `periodicDim == 3` with a vacuum axis while sampling only two k directions, and those are accepted; bulk 3D meshes are rejected). By default the 2 bands closest to E_f are plotted; the sidebar band picker selects specific bands or the N closest. Derived data survives GUI re-parses (frame changes, revert). v1.2.11 correctness pass: TR-reduced half meshes are detected (per-axis closing-gap validation) and unfolded for single-spin calculations, shifted grids interpolate correctly across their closing cell, the surface domain is derived from the two active mesh axes (one reciprocal primitive cell) and validated against the sampled 2D plane, the plot rasterizes with a CPU z-buffer (crossing bands, depth-tested axes/Fermi plane, E_f domain gating), axes are labelled k₁/k₂, the energy axis scales relative to the reciprocal-plane extent, export/print honors the interactive rotation (persisted `bandSurfaceOrientation`), and persisted surfaces are validated on decode and before drawing. v1.2.12/1.2.13 correctness rounds: TR unfolding negates the FULL k-vector on ANY reduced axis (y/z included, degenerate axes must be TR-invariant), reduced meshes are rejected for magnetic/SOC/noncollinear calculations via a parser-set `timeReversalSymmetric` gate, the base plane no longer writes depth (coplanar bands at `energyMin` render instead of z-fighting), and the translucent base/Fermi planes are rasterized as single quads (no shared-diagonal double blend). v1.2.14 TR-classification hardening: `timeReversalSymmetric` defaults to false (unknown) for metadata-less structures and legacy state files, TR markers are scanned only from the last `Program PWSCF` banner (earlier calculations in concatenated output don't veto the selected mesh), and echoed `lspinorb`/`total magnetization` values are parsed so `.false.`/0.00 echoes don't falsely break TR. v1.2.15: zero-magnetization noncollinear/SOC runs preserve TR (QE INPUT_PW enforces TRS), so the mode only vetoes unfolding when the run is actually magnetized (nonzero starting/total magnetization or atomic moments); the calculation boundary accepts both banner spellings (starts form and bare version form) while excluding mention/exit lines.
- [x] Band-surface full-grid reconstruction and fs.x input (v1.2.16): QE symmetry matrices (with per-operation time reversal) and the echoed `K_POINTS` grid spec are parsed, and symmetry-reduced wedges are expanded to the full Monkhorst-Pack grid with the fs.x `fill_fs_grid` reconstruction (rotational part only, global plus per-operation TR, half-step shift convention, non-uniform-weight wedge upgrade). A 2D-slab fs.x `.bxsf` (vacuum axis with one node or the duplicated periodic endpoint) is sliced directly into the band-surface plot with original band numbering preserved. The plot adds a viridis colorbar, Fermi-level marker, band legend, and metric-aware reciprocal-corner labels (Γ-X-Y-M quadrant for square/rectangular, Γ/Γ+b₁/Γ+b₂/Γ+b₁+b₂ for hexagonal), with an E_f ± 1 eV default band window (VBM/CBM manifolds for insulators) and sheet-count-adaptive sampling density.
