...
Known asymmetric-unit-only inputs are not silently treated as complete crystals. CIF declared-operation inputs and numeric or unambiguous symbolic CRYSCAL `CRYSTAL` groups 1–230 are expanded to complete cells during parsing. Unknown or ambiguous symbols are never guessed and remain incomplete. CRYSCAL `SLAB` records first use the completed bulk structure, then construct a bounded primitive hkl surface cell with the requested number of atomic planes and vacuum; the result is 2D periodic. `POLYMER` input is represented as a 1D-periodic crystal in a finite nonsingular embedding. Symmetry and canonical 3D k-path analysis remain unavailable for both lower-dimensional forms.

CRYSCAL symbolic space-group matching uses a database-backed normalized Hermann–Mauguin (HM) lookup: case, whitespace, `-`, `_`, and `/` are ignored before comparison. A symbol is accepted for expansion only when it resolves unambiguously to one group. Database operation lookup selects a Hall setting compatible with the parser's cell convention: hexagonal `H` for rhombohedral R groups, unique-b for monoclinic groups, and the canonical lowest Hall setting for other systems. If a required compatible setting is unavailable, expansion fails descriptively rather than marking an incompatible result complete.
...
QE band-card and Wannier90 export behavior is:
...
- **QE `K_POINTS tpiba_b` card body** (`KPathExport.qeKPointsTpibaB`). Sampling weights, final-row handling, and weight-0 disconnected jumps are identical to `crystal_b`. Fractional route nodes are converted through the active conventional reciprocal basis to Cartesian `2π/alat` coordinates using the explicit stable convention `alat = |cell.a|`; the tooltip exposes this convention because the receiving QE input must use the same `alat`. Missing, singular, non-finite, or unrepresentable cell/route data is rejected. The UI writes the card body only to `kpath.tpiba_b`.
...
- **Sidebar actions.** All six formats are offered in two rows — QE (.pwscf), QE crystal_b, QE tpiba_b, then Wannier90, kpf, and VASP — with per-format default filenames (`kpath.qe`, `kpath.crystal_b`, `kpath.tpiba_b`, `kpath.win`, `kpath.kpf`, `KPOINTS`) and save-panel wiring; a button that cannot encode the current route is disabled with an explanatory tooltip.
...
- All imported coordinates are fractional (crystal) coordinates in the conventional reciprocal basis. QE data lines may carry an optional weight column (validated but not stored). VASP Cartesian line-mode is rejected. QE `automatic`/`gamma` grids are not band paths; `tpiba_b` is correctly identified as a Cartesian band path but import is rejected because conversion requires active cell/`alat` context, with guidance to use crystal coordinates.
