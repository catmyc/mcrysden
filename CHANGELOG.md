...
- **Band-surface plots (`--band-surf`)** — a true 3D surface plot in 3D axes (k_x, k_y, E) for QE calculations on uniform k-point meshes, **limited to 2D systems**: the mesh must be a detected 2D k-grid (a slab with exactly two non-degenerate sampling axes); bulk 3D meshes are rejected with a clear error. The k_x/k_y axes are drawn in the rotated frame with ticks (physical Å⁻¹ units when the source cell is known, fractional otherwise), a base plane, and a vertical E axis; surfaces are shaded viridis sheets with a translucent Fermi plane, drag-to-rotate, printing, and PNG/PDF/SVG/EPS/PS export. By default only the **2 bands closest to the Fermi level** are plotted; a sidebar band picker (Electronic Structure ▸ Band surface bands) selects specific bands via checkboxes or re-selects the N closest with a stepper. The surface region defaults to the first non-collinear plane of the scene's high-symmetry route (canonical, or user-defined via `--kpath`). Requires a `.pwo`/`.out`/`.bands` input with `bands (ev):` mesh data.
