...
enum ProjectStore {
    ...
    static func load(from url: URL) throws -> Scene {
        ...
        // 200 MB is a generous but bounded ceiling. The check is fail-closed:
        ...
        do {
            ...
            guard let number = attributes[.size] as? NSNumber else {
                throw ProjectStoreError.malformed("could not determine project file size")
            }
            fileSize = number.int64Value
        } catch let error as ProjectStoreError {
            throw error
        } catch {
            throw ProjectStoreError.malformed("could not inspect project file: \(error.localizedDescription)")
        }
        if fileSize < 0 {
            throw ProjectStoreError.malformed("project file has an invalid negative size")
        }
